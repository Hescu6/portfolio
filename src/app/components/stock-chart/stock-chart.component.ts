...
export class StockChartComponent implements OnInit {
  ...
  chartItCandle(data: Object) {
    ...
    const w = 375 - margin.left - margin.right;
    ...
    let xBand = d3
      .scaleBand().domain(d3.range(-1, dates.length))
      .range([0, w]).padding(0.3);

    var xAxis = d3
      .axisBottom().scale(xScale)
      .ticks(6).tickFormat(d => dateFormatter(d));

    // add clip path to focus - needed to make chart draggable
    focus
      .append("rect").attr("id", "rect")
    ...
    // Add x-axis to chart
    var gX = focus
      .append("g").attr("class", "axis x-axis") //Assign "axis" class
    ...
    var chartBody = focus
      .append("g").attr("class", "chartBody")
      .style("pointer-events", "all").attr("clip-path", "url(#clip)");

    // draw candles
    let candles = chartBody
      .selectAll(".candle").data(prices).enter()
      .append("rect").attr("x", (d, i) => xScale(i) - xBand.bandwidth())
      .attr("class", "candle").attr("y", d => yScale(Math.max(d.open, d.close)))
    ...
    let stems = chartBody
      .selectAll("g.line").data(prices)
      .enter().append("line")
      .attr("class", "stem").attr("x1", (d, i) => xScale(i) - xBand.bandwidth() / 2)
    ...
    var zoom = d3
      ...
      .translateExtent(extent).extent(extent)
      .on("zoom", zoomed).on("zoom.end", zoomend);

    // initialise brush state
    context
      .append("g").attr("class", "brush")
      .on("click", brushed).call(brush)
      .call(brush.move, [w - (dates.length / w) * 200, w]);
